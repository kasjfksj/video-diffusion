...
class VAE(nn.Module):
    """Variational Autoencoder for video compression"""
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim=None):
        ...
        self.temporal_conv = nn.Conv3d(dim_out, dim_out, 
                                       kernel_size=(3, 1, 1), 
                                       padding=(1, 0, 0))
...
class SpatioTemporalAttention(nn.Module):
    ...
    def forward(self, x):
        b, c, f, h, w = x.shape
        # Process all spatial-temporal tokens jointly
...
class Unet3D(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                Residual(PreNorm(dim_out, SpatioTemporalAttention(dim_out, heads=attn_heads))) if use_sparse_linear_attn else nn.Identity(),
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                Residual(PreNorm(dim_in, SpatioTemporalAttention(dim_in, heads=attn_heads))) if use_sparse_linear_attn else nn.Identity(),
    ...
    def forward(self, x, time, focus_present_mask=None):
...
class GaussianDiffusion(nn.Module):
    """Base diffusion model with properly scaled guidance"""
    def __init__(
        self,
        denoise_fn,
        vae,
    ...
    def p_sample(self, x, t, clip_denoised=True, guidance_fn=None, guidance_scale=1.0):
        """
        Sample one step with optional external guidance function
        Args:
            guidance_fn: Optional function(x_t, t) -> gradient (unscaled)
            guidance_scale: Lambda parameter for guidance strength (default: 1.0)
        """
        ...
        # Apply external guidance if provided with proper scaling
        if guidance_fn is not None:
            # Get sigma_t = sqrt(1 - alpha_bar_t)
            sigma_t = extract(self.sqrt_one_minus_alphas_cumprod, t, x.shape)
            
            # Compute guidance gradient
            guidance_grad = guidance_fn(x, t)
            
            # Scale by sigma_t / lambda^2 as per the formula
            scaled_guidance = 1* (sigma_t / (guidance_scale ** 2)) * guidance_grad
            
            model_mean = model_mean + scaled_guidance
        
        return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise

    def p_sample_loop(self, shape, guidance_fn=None, guidance_scale=1.0):
        """Generate samples in latent space with optional guidance"""
    ...
    def sample(self, batch_size=16, guidance_fn=None, guidance_scale=1.0):
        """
        Generate video frames
        Args:
            guidance_fn: Optional guidance function
            guidance_scale: Lambda parameter (higher = stronger guidance)
        Returns:
            frames in pixel space [0, 1] and latents
        """
...
def temporal_consistency_guidance(model, prev_frames, overlap_frames, guidance_scale=1.0):
    ...
    def guidance_fn(x_t, t):
        ...
        current_first_pixels = (current_first_pixels.clamp(-1, 1) + 1) / 2  # Scale to [0, 1]
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        # Optimizer for diffusion model and VAE
        self.opt = Adam(
