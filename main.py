...
def main():
    ...
    config = {
        ...
        # Resume
        'resume_from': 16,  # Set to milestone number to resume
    }
    ...
    if config['resume_from'] is not None:
        trainer.load(config['resume_from'])
    trainer.train()
        # ============== START TRAINING ==============
    ...
    # Save as GIF
    save_video_as_gif(frames, f'generated_video_{a}.gif', fps=1)
    ...
    for i in range(3):  # Generate 5 chunks
        prev_frames = frames[:, :, -4:]  # Get last 4 frames in pixel space
        guidance_fn = temporal_consistency_guidance(diffusion, prev_frames, 4, 10)
        frames, latents = diffusion.sample(batch_size=1, guidance_fn=guidance_fn)
        frames = frames[:, :, 4:] 
        all_frames.append(frames)
    ...
    long_video = torch.cat(all_frames, dim=2)  # (B, C, total_F, H, W)
    save_video_as_gif(long_video, f'long_video_{a}.gif', fps=1)
